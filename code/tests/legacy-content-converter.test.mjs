...
async function runConverter(output) {
  await execFileAsync(
    ...
    [
      ...
      "--baseline",
      join(applicationRoot, "reports", "baseline", "baseline.json"),
    ],
    { cwd: applicationRoot },
  );
}
...
test("legacy converter creates a lossless, deterministic catalog import and pending-attribution report", async (t) => {
  ...
  const [
    ...
    readFile(
      join(first.contentDirectory, "courses", "legacy-import.json"),
      "utf8",
    ),
    readFile(
      join(second.contentDirectory, "courses", "legacy-import.json"),
      "utf8",
    ),
  ]);
  ...
  assert.equal(
    stages.flatMap((document) => document.projectOutcomes).length,
    9,
  );
  ...
  assert.ok(
    report.unresolved.entries.every(
      (entry) =>
        entry.missing.includes("author") && entry.missing.includes("license"),
    ),
  );
  assert.ok(
    items.every((item) => item.legacyImport.source === "learning-site/data.js"),
  );
  assert.ok(
    items.every(
