...
function parseArguments(args) {
  ...
  return {
    ...
    outputDirectory: resolve(
      options.outputDirectory ?? join(root, "reports", "baseline"),
    ),
  };
}
...
async function loadLegacyData(root) {
  ...
  if (
    ...
    throw new Error(
      `Expected ${dataPath} to assign tracks, stages, and courses to window.HubData.`,
    );
  }
...
async function inspectLegacyCapabilities(root, data) {
  ...
  return [
    ...
    },
    {
      ...
      available:
        app.includes("localStorage") && app.includes("agentHubLastDoc"),
      evidence: ["learning-site/app.js"],
    },
    {
      id: "path-audit",
...
function resolveReference(root, reference) {
  ...
  if (reference.path.startsWith("@mark/")) {
    return join(
      ...
      "assets",
      "marks",
      `${reference.path.slice("@mark/".length)}.svg`,
    );
  }
...
async function auditReferences(root, references) {
  ...
  for (const reference of references) {
    try {
      ...
      if (
        (expectedDirectory && target.isDirectory()) ||
        (!expectedDirectory && target.isFile())
      ) {
        resolved += 1;
      } else {
...
function isProjectDirectory(entries) {
  return entries.some(
    (entry) =>
      PROJECT_MARKERS.has(entry.name) || /^readme.*\.md$/i.test(entry.name),
  );
}
...
async function findProjectDirectories(root) {
  ...
  async function walk(directory, relativeDirectory = "") {
    ...
    const depth =
      relativeDirectory === "" ? 0 : relativeDirectory.split("/").length;
...
function projectIsListed(projectDirectory, references) {
  ...
  return references.some(
    (reference) =>
      ...
      (reference.path === projectDirectory ||
    ...
    nestedRepositories.map(async (repositoryPath) => {
      try {
        const { stdout } = await execFileAsync(
          ...
          [
            "-C",
            join(root, "local-courses", repositoryPath),
            "status",
            "--porcelain=v1",
        ...
        return {
          path: repositoryPath,
          workingTree: stdout.trim() === "" ? "clean" : "dirty",
        };
      } catch {
...
async function createBaselineReport(root) {
  ...
  const [coverageProjects, legacyCapabilities, localMaterials, referenceAudit] =
    await Promise.all([
  ...
  const repositories = await inspectRepositories(
    root,
    localMaterials.nestedRepositories,
  );
...
async function main() {
  ...
  console.log(
    `Wrote ${basename(reportPath)} and ${basename(summaryPath)} to ${options.outputDirectory}`,
  );
}
