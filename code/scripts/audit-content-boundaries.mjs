...
function parseArguments(args) {
  ...
  return {
    ...
    outputDirectory: resolve(
      options.outputDirectory ?? join(root, "reports", "content-boundaries"),
    ),
  };
}
...
function assertStringArray(value, field) {
  if (
    ...
    throw new Error(
      `Policy field ${field} must be an array of non-empty strings.`,
    );
  }
}
...
function validatePolicy(policy) {
  ...
  for (const boundary of policy.ownership) {
    if (
      ...
      throw new Error(
        "Each ownership boundary requires path, classification, git, image, and deployment.",
      );
    }
  }
  ...
  assertStringArray(
    policy.forbiddenArtifactPrefixes,
    "forbiddenArtifactPrefixes",
  );
  assertStringArray(
    policy.forbiddenArtifactPatterns,
    "forbiddenArtifactPatterns",
  );
  assertStringArray(
    policy.trackedLocalMaterialAllowlist,
    "trackedLocalMaterialAllowlist",
  );
}
...
function forbiddenArtifactReason(path, policy) {
  const prefix = policy.forbiddenArtifactPrefixes.find((candidate) =>
    path.startsWith(candidate),
  );
  if (prefix) {
    return `matches forbidden artifact prefix ${prefix}`;
  }
  const pattern = policy.forbiddenArtifactPatterns.find((candidate) =>
...
async function trackedLocalMaterialFiles(root) {
  try {
    const { stdout } = await execFileAsync(
      "git",
      ["-C", root, "ls-files", "-z", "--", "local-courses"],
      {
        encoding: "utf8",
...
async function createReport(root, policyPath) {
  ...
  const [ciUnsafe, gitIgnoreRules, dockerIgnoreRules, trackedLocalMaterials] =
    await Promise.all([
  ...
  const violations = [
    ...checks.gitIgnore.missing.map(
      (rule) => `Missing Git ignore rule: ${rule}`,
    ),
    ...checks.dockerIgnore.missing.map(
      (rule) => `Missing Docker ignore rule: ${rule}`,
    ),
    ...checks.ciArtifacts.unsafe.map(
      (artifact) =>
        `Unsafe CI artifact path: ${artifact.path} (${artifact.file})`,
    ),
...
async function main() {
  ...
  console.log(
    `Wrote ${basename(jsonPath)} and ${basename(markdownPath)} to ${options.outputDirectory}`,
  );
