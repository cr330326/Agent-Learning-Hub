  ...
  return {
    ...
    baselinePath: resolve(
      options.baselinePath ??
        join(root, "reports", "baseline", "baseline.json"),
    ),
  };
}
...
function toReference(link) {
  if (
    ...
    throw new Error(
      "Expected every legacy course link to contain a label and target.",
    );
  }
...
function accessPolicyFor(references) {
  const localPath =
    references.find((reference) => reference.localPath !== null)?.localPath ??
    null;
  const sourceUrl =
    references.find((reference) => reference.sourceUrl !== null)?.sourceUrl ??
    null;
...
function loadLegacyDataFromSource(source, dataPath) {
  ...
  if (
    ...
    throw new Error(
      `Expected ${dataPath} to assign the legacy HubData collections.`,
    );
  }
  ...
  return legacyStages.map((legacyStage, stageIndex) => {
    ...
    const learningItemIds = unique(
      (legacyStage.reading ?? []).map((reading) => {
        ...
        if (!itemId) {
          throw new Error(
            `No converted reading item exists for ${reading.doc}.`,
          );
        }
        return itemId;
      }),
...
function createProjectOutcomes(legacyProjects) {
  return legacyProjects.map((project, projectIndex) => {
    if (!Array.isArray(project) || project.length < 3) {
      throw new Error(
        "Expected every legacy project to contain a level, title, and summary.",
      );
    }
    ...
    if (!Number.isInteger(level) || level < 1) {
      throw new Error(
        `Could not determine a project level from ${project[0]}.`,
      );
    }
  ...
  const mismatches = Object.entries(expected)
    ...
    .concat(
      Object.entries(expected)
        .filter(
          ([key, value]) => Number.isInteger(value) && value !== actual[key],
        )
        .map(
          ([key, value]) =>
            `Expected ${key}=${value}, received ${actual[key]}.`,
        ),
    );
...
async function main() {
  ...
  const counts = {
    ...
    stageTasks: stageDocuments.flatMap((document) => document.stageTasks)
      .length,
    stageOutcomes: stageDocuments.flatMap(
      (document) => document.projectOutcomes,
    ).length,
  ...
  if (!baselineComparison.matches) {
    throw new Error(
      `Baseline reconciliation failed: ${baselineComparison.mismatches.join(" ")}`,
    );
  }
  ...
  await Promise.all([
    ...
    writeJson(
      join(options.contentDirectory, "stages", "legacy-import.json"),
      stageDocuments,
    ),
    writeJson(
      join(options.contentDirectory, "courses", "legacy-import.json"),
      items,
    ),
    writeJson(
