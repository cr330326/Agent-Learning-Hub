...
**查询参数校验**：轨道、阶段、访问方式和标签都先对照目录中真实存在的值检查，不匹配即视为未传，并且不参与分页链接。这条约束有两个来由：一是未校验的值会被当成筛选条件执行，把一个 515 条的目录悄悄变成空网格，读者无从判断是没有内容还是链接坏了；二是曾经把原始参数直接印在结果计数旁边，等于把任意查询串回显进页面。
...
**一个目标一条结果。** 旧站导入把每个上游章节文件都升格成了独立课程条目（514 条中有 472 条是 `reading-chapter`），这些条目的唯一 reference 与条目同名、指向同一个文件。早期的索引对每个条目既产出 `item` 文档又产出 `local-chapter` 文档，于是本地模式下每条结果都成对出现，一次 `agent` 查询返回 619 行而真实目标不到一半。
...
结果行的定位信息取自 `SearchDocument.summary`：条目用自身摘要、阶段用阶段摘要、章节用所属条目标题。此前该行只显示关联阶段，而 515 条里只有 18 条挂了阶段，整页结果都塌成同一句“未关联路线阶段”。
