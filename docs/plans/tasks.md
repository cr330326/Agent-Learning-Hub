...
**状态**：实施中（核心产品功能与原生双模式验收已完成；本地 Docker 新鲜构建、T8.8 内容策展和完整云端/外部运维仍待完成）
**版本**：1.0  
**日期**：2026-08-24
...
**目标主机云端模式验证（2026-08-20）**：SSH 恢复后在 `VM-0-9-ubuntu` 上以 **Cloud Mode** 实跑了发布镜像 `v0.1.0`（linux/amd64，digest `sha256:76da5d2a…`），只绑 `127.0.0.1:3000`，从不对公网暴露；证据见 [cloud-host-verify](../../code/reports/cloud-host-verify/cloud-host-verify.md)。

镜像经 `docker save | ssh docker load` 传输——GHCR 包是私有的，主机未认证，`docker pull` 返回 `denied`。

通过项目自己的公开冒烟套件（首页、9 条公开路由、健康检查、管理员边界）；匿名对 `/api/state`、`/api/data`、`/api/admin/health` 全部 401；`/api/local-image` 404；`/read/<local-preferred>` 返回 200 但渲染的是「READER / SAFE FALLBACK — 这个条目当前没有站内正文」，按内容核对**零第三方正文泄漏**（五个正文特征串一个都不出现），导航正确显示「登录 / 云端模式」。镜像内不含 `local-courses` 与 `*.sqlite`，在主机上复核了 GATE-09。

验证后已拆除：把配着占位 OAuth 凭据的容器留在生产主机上，容易被误认成真实部署。镜像保留在主机，真正部署时无需再传。

**遗留**：公网 DNS、Caddy TLS、真实 GitHub OAuth App，以及随之而来的整个鉴权侧（登录、学习状态、导出、删号）在主机上仍未验证——它们需要维护者持有的域名与凭据。鉴权侧已在开发机与容器间拓扑由 `test:e2e:cloud` 覆盖（29/29，见 T8.12）。
...
**目标主机演练（2026-08-20）**：在腾讯云 Lighthouse 主机 `VM-0-9-ubuntu`（Ubuntu 24.04、x86_64）上执行 `npm run drill:restore`，**16/16 通过，耗时 3.0s**。演练在固定版本的维护容器 `node:24.18.0-bookworm` 内运行，与 `lighthouse-deploy.sh backup` 同形；证据见 [restore-drill-host](../../code/reports/restore-drill-host/restore-drill.md)，记录了日期、耗时、主机平台（`linux/x64`）、node 与应用版本。

覆盖 T7.5 要求的三件事：干净环境恢复（目标目录从未存在过数据库、`-wal`/`-shm` 均不存在）、完整性与迁移检查（`integrity_check` 为 `ok`，再用应用自己的 opener 重开并读出每个用户，即跑过迁移）、以及用户关联与全部私有数据的逐表核对（users/accounts/sessions/item_progress/stage_task_progress/notes/bookmarks/stage_outcomes 八张表行数与源库完全一致）。三组反向对照——错误口令、被翻掉一个字节的密文、覆盖已有目标——连同"缺 `--yes`"的确认门全部被正确拒绝。
...
**本次复核补充（2026-08-24）**：已再次同步本任务要求的根 [README](../../README.md)、[AGENTS](../../AGENTS.md)、[GUIDE](../../GUIDE.md)、[local-courses README](../../local-courses/README.md)、[spec](./spec.md)、[plan](./plan.md) 与本文件，并修正动态素材漂移示例、旧的任务计数和 T8.8 完成状态。项目相关 8 份文档的链接路径检查、全仓库 Prettier 和 `git diff --check` 均通过；全仓库链接脚本仍会报告 `.agents/` 与 `.claude/` 内既有技能示例的外部/占位链接，未改动这些非项目文档。T7.7 仍不勾选，因为其完成定义还包含依赖 T7.3 的云端流程可复现。
...
**当前复核（2026-08-24）**：最新 [catalog-drift 报告](../../code/reports/materials/catalog-drift.md) 列出 4 个未收录仓库（新增 `Agentic/HelloAgents`），0 条失效路径；数量随素材库变化，以报告为准。T8.10 的工具和边界仍然有效，但不替代人工目录归属决定。
...
## 12.1 剩余工作与解除条件（2026-08-24 核实）

当前有 10 个未完成任务。T7.5（GATE-07）已经完成；T8.8 是仍未完成的非云端内容策展任务，另外 9 个主要卡在仓库之外的四类前置条件上。实现与本地测试证据已经分开记录，不能把外部条件或目录归属决策隐含标记为完成：
...
| T8.8 本地课程章节策展              | 先决定重复正文的唯一目录拥有者，再补章节声明                             | ADR + 唯一目标审计                                               |
