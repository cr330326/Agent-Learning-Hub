...
does not claim release acceptance: real GitHub account restoration, an actual
published-image rollout, off-host backup recovery and production rollback still
...
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| AC-01 Cloud without local materials          | Cloud production HTTP smoke passed on `127.0.0.1:3231`; GitHub authorize redirect used `read:user`; `/api/auth/get-session` returned `null`; the old callback returned `410`. On 2026-08-09, `agent-learning-hub:docs-verify` built from `code/docker/Dockerfile`, started in isolated Cloud Mode on `127.0.0.1:3418`, passed health and 9-route public E2E checks, and exposed only the SQLite state volume. Image inspection found no `local-courses`, SQLite file, `.data`, or reports; cloud search returned no `LOCAL-CHAPTER` or local reader route. | Verified at container/public-flow level |
...
docker build --progress=quiet --file code/docker/Dockerfile \
  --tag agent-learning-hub:docs-verify .
APP_IMAGE=agent-learning-hub:docs-verify APP_PORT=3417 \
COMPOSE_PROJECT_NAME=agent-learning-hub-docs-verify \
code/scripts/docker-deploy.sh local up
APP_URL=http://127.0.0.1:3417 EXPECTED_RUNTIME_MODE=local \
npm run test:e2e:local --prefix code
APP_IMAGE=agent-learning-hub:docs-verify APP_PORT=3418 \
COMPOSE_PROJECT_NAME=agent-learning-hub-cloud-verify \
code/scripts/docker-deploy.sh cloud up
APP_URL=http://127.0.0.1:3418 EXPECTED_RUNTIME_MODE=cloud \
npm run test:e2e --prefix code
git diff --check
```
