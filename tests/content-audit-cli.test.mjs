...
const tsxPath = join(repositoryRoot, "code", "node_modules", ".bin", "tsx");
const auditCommandPath = join(repositoryRoot, "code", "scripts", "audit-content.ts");
...
test("content audit CLI writes Cloud JSON and Markdown reports", async (t) => {
  ...
  const markdown = await readFile(join(outputDirectory, "content-audit-cloud.md"), "utf8");
...
test("content audit CLI exits unsuccessfully after writing a deterministic error report", async (t) => {
  ...
  await cp(join(repositoryRoot, "content"), contentRoot, { recursive: true });
