...
test("baseline-report writes a machine-readable and Markdown snapshot for the legacy site", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-baseline-"));
  const outputDirectory = join(fixtureRoot, "reports", "baseline");
  t.after(() => rm(fixtureRoot, { force: true, recursive: true }));

  await mkdir(join(fixtureRoot, "learning-site", "scripts"), { recursive: true });
  await mkdir(join(fixtureRoot, "local-courses", "Learning", "course-a"), { recursive: true });
  await mkdir(join(fixtureRoot, "local-courses", "Agentic", "course-b", ".git"), {
    recursive: true,
  });
  ...
  await writeFile(join(fixtureRoot, "learning-site", "app.js"), "legacy app", "utf8");
  await writeFile(join(fixtureRoot, "learning-site", "index.html"), "<main>Legacy site</main>", "utf8");
  await writeFile(join(fixtureRoot, "learning-site", "styles.css"), "@media (max-width: 600px) {}", "utf8");
  await writeFile(join(fixtureRoot, "learning-site", "scripts", "audit_paths.py"), "# audit", "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "Learning", "course-a", "README.md"), "A", "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "Learning", "course-a", ".git"), "gitdir: ignored", "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "Agentic", "course-b", "lesson.md"), "B", "utf8");
  ...
  const report = JSON.parse(await readFile(join(outputDirectory, "baseline.json"), "utf8"));
...
test("baseline-report marks missing local references and project directories absent from the catalog", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-coverage-"));
  const outputDirectory = join(fixtureRoot, "reports", "baseline");
  t.after(() => rm(fixtureRoot, { force: true, recursive: true }));

  await mkdir(join(fixtureRoot, "learning-site"), { recursive: true });
  await mkdir(join(fixtureRoot, "local-courses", "Learning", "listed"), { recursive: true });
  await mkdir(join(fixtureRoot, "local-courses", "Learning", "unlisted"), { recursive: true });
  await writeFile(
    join(fixtureRoot, "learning-site", "data.js"),
    `
      window.HubData = {
  ...
  await writeFile(join(fixtureRoot, "local-courses", "Learning", "listed", "README.md"), "listed", "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "Learning", "unlisted", "README.md"), "unlisted", "utf8");
  ...
  const report = JSON.parse(await readFile(join(outputDirectory, "baseline.json"), "utf8"));
  assert.deepEqual(report.referenceAudit.missing, [{ kind: "doc", path: "Learning/missing.md" }]);
...
test("baseline-report marks nested material repositories with uncommitted changes", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-dirty-repository-"));
  const outputDirectory = join(fixtureRoot, "reports", "baseline");
  const materialRepository = join(fixtureRoot, "local-courses", "Learning", "dirty-course");
  t.after(() => rm(fixtureRoot, { force: true, recursive: true }));
  ...
  const initialized = spawnSync("git", ["init", "--quiet", materialRepository], { encoding: "utf8" });
  assert.equal(initialized.status, 0, initialized.stderr);
  await writeFile(join(materialRepository, "pending.md"), "uncommitted", "utf8");
  ...
  const report = JSON.parse(await readFile(join(outputDirectory, "baseline.json"), "utf8"));
  assert.deepEqual(report.repositories, [{ path: "Learning/dirty-course", workingTree: "dirty" }]);
...
test("baseline-report records the legacy capability baseline used for Phase 8 parity checks", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-capabilities-"));
  const outputDirectory = join(fixtureRoot, "reports", "baseline");
  t.after(() => rm(fixtureRoot, { force: true, recursive: true }));
