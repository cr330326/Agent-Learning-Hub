...
const policy = {
  ...
  gitIgnoreRules: ["/local-courses/**", "/data/**", "/backups/**", ".env*", "*.sqlite", "*.sqlite-*"],
  dockerIgnoreRules: ["local-courses/", "data/", "backups/", ".env*", "*.sqlite", "*.sqlite-*"],
...
test("content-boundary audit writes a passing report for protected source and image contexts", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-boundaries-"));
  ...
  await writeFile(join(fixtureRoot, "docs", "content-boundaries.json"), JSON.stringify(policy), "utf8");
  await writeFile(join(fixtureRoot, ".gitignore"), `${policy.gitIgnoreRules.join("\n")}\n`, "utf8");
  await writeFile(join(fixtureRoot, ".dockerignore"), `${policy.dockerIgnoreRules.join("\n")}\n`, "utf8");
  ...
  const report = JSON.parse(await readFile(join(outputDirectory, "content-boundaries.json"), "utf8"));
  ...
  const summary = await readFile(join(outputDirectory, "content-boundaries.md"), "utf8");
...
test("content-boundary audit fails when a CI artifact would expose local material, state, or secrets", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-boundary-leak-"));
  const outputDirectory = join(fixtureRoot, "reports", "boundaries");
  t.after(() => rm(fixtureRoot, { force: true, recursive: true }));
...
test("content-boundary audit rejects tracked local material except explicit metadata", async (t) => {
  const fixtureRoot = await mkdtemp(join(tmpdir(), "agent-learning-hub-tracked-material-"));
  const outputDirectory = join(fixtureRoot, "reports", "boundaries");
  t.after(() => rm(fixtureRoot, { force: true, recursive: true }));
  ...
  await writeFile(join(fixtureRoot, ".gitignore"), `${policy.gitIgnoreRules.join("\n")}\n`, "utf8");
  await writeFile(join(fixtureRoot, ".dockerignore"), `${policy.dockerIgnoreRules.join("\n")}\n`, "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "README.md"), "library metadata", "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "third-party.md"), "material body", "utf8");
  await writeFile(join(fixtureRoot, "local-courses", "第三方资料.md"), "material body", "utf8");
  const initialized = spawnSync("git", ["init", "--quiet", fixtureRoot], { encoding: "utf8" });
  assert.equal(initialized.status, 0, initialized.stderr);
  const added = spawnSync(
    "git",
    ["-C", fixtureRoot, "add", "-f", "local-courses/README.md", "local-courses/third-party.md", "local-courses/第三方资料.md"],
    { encoding: "utf8" },
  );
